...
#This file is an analysis and ranking for runningbacks

college<-read.csv(file="collegeCFFdata.csv")
college<-college[
  college$pff_GAMEPOSITION=="HB"|
    college$pff_GAMEPOSITION=="RB",]

pro<-read.csv(file="proPFFdata.csv")
pro<-pro[
  pro$pff_GAMEPOSITION=="HB"|
    pro$pff_GAMEPOSITION=="RB",]
...
#12 (pff_RUN), 14 (pff_RECEIVING), 15 (pff_PASSBLOCK), 16 (pff_RUNBLOCK), 22 (pff_KICKRETURN), 23 (pff_PUNTRETURN), 27 (pff_STPENALTY), 28 (pff_STKICKOFF), 29 (pff_STPUNT)
...
write.csv(data.frame(Rank1=ranknames_1,Rank2=ranknames_2,Rank3=ranknames_3,Rank4=ranknames_4),file="Rankings_RB.csv")
